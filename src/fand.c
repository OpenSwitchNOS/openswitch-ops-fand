...
/* Pointer to fand_sybsystem_class implementation. */
static const struct fand_subsystem_class *subsystem_class;

/* Pointer to fand_sybsystem_class implementation. */
static const struct fand_fan_class *fan_class;
    ...
    SHASH_FOR_EACH_SAFE(node, next, &subsystem_data) {
        ...
        if (subsystem->marked == false) {
            ...
            SHASH_FOR_EACH_SAFE(fan_node, fan_next, &subsystem->subsystem_fans) {
                ...
                fan_class->fand_fan_destruct(fan);
                /* free the allocated data */
                free(fan->name);
                fan_class->fand_fan_dealloc(fan);
            }
            subsystem_class->fand_subsystem_destruct(subsystem);
            free(subsystem->name);
            subsystem_class->fand_subsystem_dealloc(subsystem);
...
fand_init(const char *remote)
{
    ...
    fand_plugin_init();
    subsystem_class = fand_subsystem_class_get();
    fan_class = fand_fan_class_get();
...
fand_read_status(struct ovsdb_idl *idl)
{
    ...
    SHASH_FOR_EACH(node, &subsystem_data) {
        ...
        SHASH_FOR_EACH(fan_node, &subsystem->subsystem_fans) {
            ...
            if (fan_class->fand_status_get(fan, &fan->status)) {
                VLOG_ERR("Failed reading fan status subsystem %s fan %s",
                         subsystem->name,
                         fan->name);
            }
            if (fan_class->fand_direction_get &&
                fan_class->fand_direction_get(fan, &fan->direction)) {
                VLOG_ERR("Failed reading fan direction subsystem %s fan %s",
                         subsystem->name,
                         fan->name);
            }
            if (fan_class->fand_rpm_get &&
                fan_class->fand_rpm_get(fan, &fan->rpm)) {
...
fand_reconfigure(struct ovsdb_idl *idl)
{
    ...
    OVSREC_SUBSYSTEM_FOR_EACH(cfg, idl) {
        ...
        SHASH_FOR_EACH(fan_node, &subsystem->subsystem_fans) {
            fan = (struct locl_fan *)fan_node->data;
            if (fan_class->fand_speed_set(fan, subsystem->fan_speed)) {
                VLOG_ERR("Failed setting speed subsystem %s fan %s",
                         subsystem->name,
                         fan->name);
            }
