...
struct plugin_class {
    ...
    const struct fand_fru_class *(*fru_class_get)(void);
};
...
plugin_open(const char *filename, void *data)
{
    ...
    rc = !(pc->plugin_init = lt_dlsym(dlhandle, "fand_plugin_init"));
    CHECK_LT_RC(rc, "Load fand_plugin_init");

    rc = !(pc->plugin_deinit = lt_dlsym(dlhandle, "fand_plugin_deinit"));
    CHECK_LT_RC(rc, "Load fand_plugin_deinit");

    rc = !(pc->plugin_run = lt_dlsym(dlhandle, "fand_plugin_run"));
    CHECK_LT_RC(rc, "Load fand_plugin_run");

    rc = !(pc->plugin_wait = lt_dlsym(dlhandle, "fand_plugin_wait"));
    CHECK_LT_RC(rc, "Load fand_plugin_wait");

    rc = !(pc->subsystem_class_get = lt_dlsym(dlhandle, "fand_subsystem_class_get"));
    CHECK_LT_RC(rc, "Load fand_subsystem_class_get");

    rc = !(pc->fan_class_get = lt_dlsym(dlhandle, "fand_fan_class_get"));
    CHECK_LT_RC(rc, "Load fand_fan_class_get");

    rc = !(pc->fru_class_get = lt_dlsym(dlhandle, "fand_fru_class_get"));
    CHECK_LT_RC(rc, "Load fand_fru_class_get");
...
fand_subsystem_class_get(const char *platform_type)
{
    lt_dlhandle iter_handle = 0;
    struct plugin_class *pc = NULL;
...
fand_fan_class_get(const char *platform_type)
{
    lt_dlhandle iter_handle = 0;
    struct plugin_class *pc = NULL;
...
const struct fand_fru_class *
fand_fru_class_get(const char *platform_type)
{
    lt_dlhandle iter_handle = 0;
    struct plugin_class *pc = NULL;
